<template>
  ...
  <div class="container">
    <div class="actions_row">
        <div class="act_back">
            <a class="button" v-show="page!=0" @click="prevPage()" align=left>{{translations[lang].back}}</a>
        </div>
        <div class="act_search">
            <input type="text" :placeholder="translations[lang].search" v-on:input="doSearch()" v-model="search_q">
        </div>
        <div class="act_next">
            <a class="button" v-show="page < (totalPages - 1)" @click="nextPage()">{{translations[lang].next}}</a>
        </div>
    </div>

    <!-- Tracks -->
    <div v-show="!nothingFound && !loading" class="track" v-for="track in tracks" :key="track.id">
            <img v-bind:src="track.image_medium" @click="requestTrack(track)">
            <div class="meta">{{ track.meta }}</div>
            <a class="button" @click="requestTrack(track)">{{translations[lang].requestTrack}}</a>
    </div>
            ...
            <div class="modal-wrapper" >
                <div class="modal-container">
                    ...
                    <div class="modal-body" v-if="selectedTrack">
                        ...
                        <div v-if="allowMessages" class="feedback">
                    ...
                    <div class="modal-footer">
                        <button class="button" v-if="selectedTrack" @click="sendRequest()">{{translations[lang].trackrequestModalSubmit}}</button>
...
export default {
    ...
    return {
      translations: {
        en: {
          ...
          trackRequestSuccessfull: "Your track request has been sent successfully.",
          trackrequestModalClose: "Close",
          trackRequestFailure: "Request failed, please try again later.",
          trackRequestBlocked: "This track has already been requested recently, please try again later.",
          trackRequestBlockedIp: "You have already made a request recently, please try again later.",
        },
        ru: {
          back: "Назад",
          next: "Вперед",
          ...
          trackRequestBlocked: "Этот трэк уже был заказан недавно, попробуйте повторить запрос позже.",
          trackRequestBlockedIp: "Вы уже заказывали трэк недавно, попробуйте повторить запрос позже.",
        }
      },
      tracksNum: 10,
      page: 0,
  ...
  methods: {
    resetRequestData(){
        this.trackRequestUnsuccessfull = this.trackRequestSuccessfull =
        this.trackRequestBlocked = this.trackRequestBlockedIp = this.trackRequestFailure = false;
    },
    sendRequest(){
    ...
    updateCharactersLeftName() {
        this.charactersLeft = this.maxMessageLength - (this.name.length + this.message.length);
    ...
    loadTracks() {
      let url = `${this.apiBase}/music/?limit=${this.tracksNum}&offset=${this
        .tracksNum * this.page}&search_q=${this.search_q}&server=${this
        .serverId || 1}&with_tags_only=true`;
      ...
      axios
        ...
        .then(response => {
          if (
            ...
            this.totalPages = Math.ceil(response.data.count / this.tracksNum);
